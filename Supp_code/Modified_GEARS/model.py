...
class First_Sub_task_MLP(torch.nn.Module):
...
class GEARS_Model(torch.nn.Module):
    ...
    def __init__(self, args):
        ...
        self.first_sub_task = First_Sub_task_MLP([hidden_size, hidden_size // 2, 1], last_layer_act='Sigmoid')
    ...
    def forward(self, data):
        ...
        if self.no_perturb:
            ...
            # print(emb_total.size()) || output shape: [batch-num_control, hidden_dim: 64(default)]
                
            #print(base_emb.size()) || output shape: [batch, gene_num, hidden_dim]
            base_emb = base_emb.reshape(num_graphs * self.num_genes, -1)
            # print(base_emb.size()) || output shape: [batch * gene_num, hidden_dim]
            base_emb = self.bn_pert_base(base_emb)

            first_sub_task_out = base_emb.reshape(num_graphs, self.num_genes, -1)
            first_sub_task_out = torch.nn.Sigmoid()(self.first_sub_task(first_sub_task_out))
            ...
            out = w + self.indv_b1 # print(out.size()) || output shape :[batch, gene_exp_dim, 1]
            
            ## second sub-task output
            second_sub_task_out = torch.nn.Sigmoid()(out)
            ...
            cross_gene_out = cross_gene_out * self.indv_w2
            cross_gene_out = torch.sum(cross_gene_out, axis=2)
            # out = torch.exp(cross_gene_out + self.indv_b2)
            # out = out.reshape(num_graphs * self.num_genes, -1)
            out = cross_gene_out + self.indv_b2
            # out = out.reshape(num_graphs * self.num_genes, -1) + x.reshape(-1,1)
            ...
            # fold_change_version
            # out = torch.exp(out)
            # our_fc = out.reshape(num_graphs * self.num_genes, -1)
            # our_fc = torch.split(torch.flatten(our_fc), self.num_genes)
            # our_fc = torch.stack(our_fc)   
            
                   
            out = out.reshape(num_graphs * self.num_genes, -1) * x.reshape(-1,1)
            out = torch.split(torch.flatten(out), self.num_genes)
            third_sub_task_out = torch.stack(out) 
                       
            # print(third_sub_task_out.size())
            ## uncertainty head
            # if self.uncertainty:
            ...
            # print(first_sub_task_out.size(), second_sub_task_out.size(), third_sub_task_out.size())|| [batch_size, num_gene, 1], [batch_size, num_gene, 1], [batch_size, num_gene]
            return first_sub_task_out, second_sub_task_out, third_sub_task_out
