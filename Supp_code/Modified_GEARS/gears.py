...
from .utils import loss_fct, uncertainty_loss_fct, parse_any_pert, \
                  ...
                  get_GI_genes_idx, get_GI_params, loss_sub_task
    ...
    def train(self, epochs = 20, 
        ...
        for epoch in range(epochs):
            ...
            for step, batch in enumerate(train_loader):
                ...
                elif self.config['Sub_tasks']:
                    pred = self.model(batch)
                    losses = loss_sub_task(pred, batch.y_1, batch.y_2, batch.y_3)
                    loss3 = loss_fct(pred[2], y, batch.pert,
                                  ctrl = self.ctrl_expression, 
                                  dict_filter = self.dict_filter,
                                  direction_lambda = self.config['direction_lambda'])
                    loss = 0.1 * losses[0] + 0.1 * losses[1] + 0.8 * loss3
                    # loss = 0.1 * losses[0] + 0.1 * losses[1] + 0.4 * losses[2] + 0.4 * loss3
            ...
            if val_metrics['mse_de'] < min_val:
                ...
                best_model = deepcopy(self.model)
            '''
            val_loss = evaluate(val_loader, self.model,
                                 self.config['uncertainty'], self.device, True, self.ctrl_expression, self.dict_filter, self.config['direction_lambda'])
            log = "Epoch {}: Validation loss: {:.4f}. "
            print_sys(log.format(epoch+1, val_loss))
            if val_loss < min_val:
                min_val = val_loss
                best_model = deepcopy(self.model)
            '''
            ...
            # Print epoch performance
            # log = "Epoch {}: Train Overall ROC-AUC1: {:.4f} " \
            #         "Train Overall PR-AUC1: {:.4f} " \
            #         "Train Overall ROC-AUC2: {:.4f} " \
            #         "Train Overall PR-AUC2: {:.4f} " \
            #         "Train Overall PCC: {:.4f} " \
            #         "Validation Overall ROC-AUC1: {:.4f} " \
            #         "Validation Overall PR-AUC1: {:.4f} " \
            #         "Validation Overall ROC-AUC2: {:.4f} " \
            #         "Validation Overall PR-AUC2: {:.4f} " \
            #         "Validation Overall PCC: {:.4f}. "
            # print_sys(log.format(epoch + 1, train_metrics['ROC-AUC1'], train_metrics['PR-AUC1'], train_metrics['ROC-AUC2'], train_metrics['PR-AUC2'], train_metrics['PCC'], 
                                #  val_metrics['ROC-AUC1'], val_metrics['PR-AUC1'], val_metrics['ROC-AUC2'], val_metrics['PR-AUC2'], val_metrics['PCC']))
        ...
        # log = "Epoch {}: Testing loss: {:.4f}. "
        # print_sys(log.format(epoch+1, test_res))
        # test_metrics = compute_metrics(test_res)    
        # log = "Best performing model: Test Top 20 DE MSE: {:.4f}"
        # print_sys(log.format(test_metrics['mse_de']))
        # log = "Epoch {}: Test Overall ROC-AUC1: {:.4f} " \
        #             "Test Overall PR-AUC1: {:.4f} "\
        #             "Test Overall ROC-AUC2: {:.4f} "\
        #             "Test Overall PR-AUC2: {:.4f} "\
        #             "Test Overall PCC: {:.4f}. "
        # print_sys(log.format(epoch + 1, test_metrics['ROC-AUC1'], test_metrics['PR-AUC1'], test_metrics['ROC-AUC2'], test_metrics['PR-AUC2'], test_metrics['PCC']))
