...
class PertData:
    ...
    def new_data_process(self, dataset_name,
                         adata = None,
                         skip_calc_de = False, sub_task_data = None):
        """
        Process new dataset
        ...
        dataset_fname = os.path.join(pyg_path, 'cell_graphs.pkl')
        print_sys("Creating pyg object for each cell in the data...")
        self.create_dataset_file(sub_task_data)
        print_sys("Saving new dataset pyg object at " + dataset_fname) 
        pickle.dump(self.dataset_processed, open(dataset_fname, "wb"))    
        print_sys("Done!")
    ...
    def create_cell_graph(self, X, y, de_idx, pert, pert_idx=None, y_1 = None, y_2 = None, y_3 = None):
        """
        ...
        return Data(x=feature_mat, pert_idx=pert_idx,
                    y=torch.Tensor(y), de_idx=de_idx, pert=pert, y_1 = torch.Tensor(y_1), y_2 = torch.Tensor(y_2), y_3 = torch.Tensor(y_3))

    def create_cell_graph_dataset(self, split_adata, pert_category,
                                  num_samples=1, sub_tasks_data = None):
        """
        ...
        if sub_tasks_data is not None:
            sub_tasks_data = sc.read_h5ad(sub_tasks_data)
            gene_idx_dict = dict(zip(list(sub_tasks_data.obs.index), list(range(len(sub_tasks_data.obs.index)))))
        ...
        if pert_category != 'ctrl':
            ...
            y_sub_tasks = sub_tasks_data[gene_idx_dict[pert_category]] # this is modification
            ...
            for cell_z in adata_.X:
                ...
                for c in ctrl_samples.X:
                    Xs.append(c)
                    ys.append(cell_z)
                    y1.append(y_sub_tasks.layers['level1'].astype('float32'))
                    y2.append(y_sub_tasks.layers['level2'].astype('float32'))
                    y3.append(y_sub_tasks.layers['level3'].astype('float32'))
        # When considering a control perturbation
        else:
            y_sub_tasks = sub_tasks_data[gene_idx_dict[pert_category]]
            pert_idx = None
            de_idx = [-1] * num_de_genes
            for cell_z in adata_.X:
                Xs.append(cell_z)
        ...
        # Create cell graphs
        cell_graphs = []
        for X, y, y_1, y_2, y_3 in zip(Xs, ys, y1, y2, y3):
            cell_graphs.append(self.create_cell_graph(X.toarray(),
                                y.toarray(), de_idx, pert_category, pert_idx, y_1.toarray(), y_2.toarray(), y_3.toarray()))

        return cell_graphs

    def create_dataset_file(self, sub_tasks_data = None):
        """
        ...
        for p in tqdm(self.adata.obs['condition'].unique()):
            self.dataset_processed[p] = self.create_cell_graph_dataset(self.adata, p, sub_tasks_data = sub_tasks_data)
        print_sys("Done!")
