...
def loss_sub_task(pred, y_1, y_2, y_3):
    first_sub_task_out = pred[0].squeeze(-1)
    second_sub_task_out = pred[1].squeeze(-1)
    # third_sub_task_out = pred[2]
    ...
    loss_binary = torch.nn.BCELoss(weight = level_1_output * loss_weights / 4 + 1)
    loss_1 = loss_binary(first_sub_task_out.squeeze(-1), level_1_output)
    
    
    with torch.no_grad():
        mask = level_1_output!=0
    with torch.no_grad():
        ...
        weights[weights > 0] -= 1
    loss_binary = torch.nn.BCELoss(weight = weights + mask, reduction = 'sum')
    loss_2 = loss_binary(second_sub_task_out, level_2_output) / torch.sum(weights + mask)

    # loss_3 = torch.sum((mask * (third_sub_task_out.squeeze(-1)-third_sub_task_out) ** 2)) / torch.sum(mask)
    
    # return loss_1, loss_2, loss_3, (first_sub_task_out, second_sub_task_out, third_sub_task_out), mask
    return loss_1, loss_2, (first_sub_task_out, second_sub_task_out), mask
